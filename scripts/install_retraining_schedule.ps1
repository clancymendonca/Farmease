...
if (-not $NotifyTelegram -and (Test-Path $EnvPath)) {
    $envContent = Get-Content $EnvPath -Raw
    if ($envContent -match "(?m)^TELEGRAM_ALERTS\s*=\s*true\s*$") {
        $NotifyTelegram = $true
    }
}
...
Write-Host "FailOnHealthIssue: $FailOnHealthIssue | NotifyTelegram: $NotifyTelegram" -ForegroundColor Cyan
